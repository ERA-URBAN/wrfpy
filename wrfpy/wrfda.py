...
class wrfda(config):
    ...
    def __init__(self, low_only=False):
        ...
        self.obsproc_dir = os.path.join(
            self.config['filesystem']['wrfda_dir'], 'var/obsproc')
    ...
    def obsproc_init(self, datestart):
        ...
        for obs in obslist:
            ...
            obsproc_nml['record2']['time_analysis'] = datetime.strftime(
    ...
    def get_obsproc_dirs(self):
        ...
        for dom in range(1, self.max_dom + 1):
            try:
                obsname = self.config['filesystem'][
                                      'obs_filename_d' + str(dom)]
    ...
    def obsproc_run(self):
        ...
        if len(self.config['options_slurm']['slurm_obsproc.exe']):
            ...
            subprocess.check_call(os.path.join(obsproc_dir, 'obsproc.exe'),
                                  cwd=obsproc_dir, stdout=utils.devnull(),
                                  stderr=utils.devnull())
    ...
    def prepare_wrfda_namelist(self, domain):
        ...
        for var in ['mp_physics', 'ra_lw_physics', 'ra_sw_physics', 'radt',
                    ...
                    'bl_pbl_physics', 'cu_physics', 'cudt',
                    'num_soil_layers']:
            # get variable from ${RUNDIR}/namelist.input
        ...
        wrfda_nml['wrfvar18']['analysis_date'] = obsproc_nml['record2'][
                                                             'time_analysis']
        wrfda_nml['wrfvar21'][
                  'time_window_min'] = obsproc_nml['record2'][
                                                   'time_window_min']
        wrfda_nml['wrfvar22'][
                  'time_window_max'] = obsproc_nml['record2'][
                                                   'time_window_max']
        if self.check_cv5_cv7():
    ...
    def wrfvar_run(self, domain):
        ...
        if len(self.config['options_slurm']['slurm_wrfvar.exe']):
            if j_id:
                ...
                wrfvar_command = ['sbatch', self.config['options_slurm'][
                                                        'slurm_wrfvar.exe']]
    ...
    def wrfda_post(self, datestart):
        '''
        Move files into WRF run dir after all data assimilation steps have
        completed
        '''
        # prepare a WRFDA workdirectory for each domain
        for domain in range(1, self.max_dom+1):
...
if __name__ == "__main__":
    datestart = datetime(2014, 7, 27, 00)
    wrf_da = wrfda()
