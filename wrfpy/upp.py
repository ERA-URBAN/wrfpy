...
class upp(config):
    ...
    def _set_variables(self):
        '''
        Define additional control variables for the unipost.exe tool,
        inherit from global config.
        '''
        self.crtm_dir = os.path.join(self.config['filesystem']['upp_dir'],
                                     'src/lib/crtm2/src/fix')
        self.post_dir = os.path.join(self.config['filesystem']['upp_dir'],
                                     'postprd')
    ...
    def _prepare_post_dir(self):
        ...
        # symlink microphysic's tables - code used is based on mp_physics
        # option used in the wrfout file
        os.symlink(os.path.join(self.config['filesystem']['wrf_run_dir'],
                                'ETAMPNEW_DATA'
                                ), os.path.join(
                                self.post_dir, 'nam_micro_lookup.dat'))
        os.symlink(os.path.join(self.config['filesystem']['wrf_run_dir'],
                                'ETAMPNEW_DATA.expanded_rain'
                                ), os.path.join(
                                self.post_dir, 'hires_micro_lookup.dat'))
...
"""
    ...
    def _run_unipost_step(self, wrfout, current_time, thours):
        ...
        subprocess.check_call(os.path.join(
                              self.config['filesystem']['upp_dir'], 'bin'
                              'unipost.exe'),
    ...
    def run_unipost_file(self, wrfout, frequency=2, use_t0=False):
        ...
        Input variables for the function are:
            ...
            - (optional) frequency: time interval in hours at which processing
               should take place
        ...
        td = [int((t - time_steps[0]).total_seconds()/3600) for t in
              time_steps]
    ...
    def _archive_output(self, current_time, thours, domain):
        ...
        shutil.move(os.path.join(self.post_dir, origname),
                    os.path.join(self.config['filesystem']['upp_archive_dir'],
                    outname))
        # check if file is indeed archived
        utils.check_file_exists(os.path.join(
                                self.config['filesystem']['upp_archive_dir'],
                                outname))
